...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      const dpr = window.innerWidth <= 768 ? 1 : Math.min(window.devicePixelRatio || 1, 1.25);
      const w = Math.floor((rect.width || parentRect?.width || window.innerWidth) * dpr);
      const h = Math.floor((rect.height || parentRect?.height || 500) * dpr);
      if (canvas.width !== w || canvas.height !== h) {
        canvas.width = w;
        canvas.height = h;
      }
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      numSquaresX.current = Math.ceil(w / (squareSize * dpr)) + 1;
      numSquaresY.current = Math.ceil(h / (squareSize * dpr)) + 1;
    };
