...
const DotField: React.FC<DotFieldProps> = memo(({
  ...
  sparkle = true,
  waveAmplitude = 1.5,
  gradientFrom = 'rgba(239, 68, 68, 0.45)',
  gradientTo = 'rgba(220, 38, 38, 0.25)',
  ...
  useEffect(() => {
    ...
    const dpr = isSmallScreen ? 1 : Math.min(window.devicePixelRatio || 1, 1.25);
    let resizeTimer: any;
    ...
    function buildDots(w: number, h: number) {
      ...
      const effectiveSpacing = window.innerWidth < 768 ? Math.max(p.dotSpacing, 22) : p.dotSpacing;
    ...
    function tick() {
      ...
      // Always maintain high baseline visibility even when mouse is still
      const targetEngagement = Math.max(0.6, Math.min(m.speed / 3, 1));
